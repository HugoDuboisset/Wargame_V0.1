...
public class FleeUnitCommandTests
{
    ...
    private FleeUnitCommandHandler CreateHandler(IDiceRoller roller)
    {
        ...
        return new FleeUnitCommandHandler(_repositoryMock.Object, withdrawalService);
    }
    ...
    [Fact]
    public async Task Handle_ForcedFlee_ShouldAutoCalculateMoves_And_SucceedRiskyAction()
    {
        var (match, u1, u2) = CreateEngagedUnits();
        
        u1.ApplyStatusEffect(StatusEffect.Routing);
        
        var cmd = new FleeUnitCommand(match.Id, u1.Id);

        var r = Roller(3); // Pass Risky action
        var handler = CreateHandler(r);

        var result = await handler.Handle(cmd, CancellationToken.None);

        result.RiskyActionFailed.Should().BeFalse();
        result.OpportunityAttacksWounds.Should().Be(0);
        
        u1.Figures.First().Position.X.Should().BeApproximately(4, 0.01);
        u1.Figures.First().Position.Y.Should().BeApproximately(10, 0.01);
